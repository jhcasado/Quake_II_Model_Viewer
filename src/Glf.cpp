...
static void DrawString(char *s, void (*funct) (char s))
{
	...
	if (m_string_center)
	{
		...
		{
			if ((fonts[curfont]->symbols[s[i]] == NULL) || (s[i] == ' ')) distance += SpaceSize;
			else
				if (i < ((int)strlen(s)-1))
					if (s[i+1] == ' ') distance += SymbolDist;
					else
					{
	...
	glPushMatrix();
	if (m_string_center) glTranslatef(-distance/2, 0, 0);
	else glTranslatef(-(1-(float)fabs(fonts[curfont]->symbols[s[0]]->leftx)), 0, 0);
	for (i=0; i<(int)strlen(s); i++)
	{
		if (s[i] != ' ') (*funct) (s[i]);
